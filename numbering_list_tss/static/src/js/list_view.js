odoo.define('numbering_list_tss.ListNumber', function (require) {
"use strict";
...
ListRenderer.include({
                ...
                if (group.groupedBy.length) {
                    ...
                    var $records = _.map(group.data, function (record,index) {
                    	if (_self.mode !== 'edit' || _self.hasSelectors){
                    		return self._renderRow(record).prepend($("<th class='o_list_serial_number_tss'>").html(index+1)); //.prepend($('<td>'));
                    	}
                    	else{
    ...
    _renderHeader: function (isGrouped) {
    	...
    	if (this.hasSelectors) {
    		$header.find("th.o_list_record_selector").before($('<th class="o_list_row_number_header o_list_serial_number_tss">').html('#'));
    		...
    		if (this.mode !== 'edit'){
    			$header.find("tr").prepend($("<th class='o_list_row_number_header o_list_serial_number_tss'>").html('#'));
    		}
    	}
    	return $header;
    },
    _renderRow: function (record) {
    	...
    	if (this.mode !== 'edit' && this.state.groupedBy.length==0){
	    	...
	    	if (index!==-1){
	    		$row.prepend($("<th class='o_list_serial_number_tss'>").html(index+1));
	    	}
    	}
    	return $row;
    },
